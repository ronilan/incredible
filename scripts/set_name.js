...
function transformCargoToml(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inBin) {
            if (lines[i].match(/^\s*path\s*=/)) {
                const match = lines[i].match(/^\s*path\s*=\s*"([^"]+)"/);
                binPath = match ? match[1] : '';
                if (pendingBinNameIdx >= 0) {
                    const bn = binPath.includes('macos.rs')
                        ? `${name}_macos`
                        : binPath.includes('windows.rs')
                            ? `${name}_windows`
                            : name;
                    lines[pendingBinNameIdx] = replaceFieldValue(lines[pendingBinNameIdx], 'name', bn);
                    pendingBinNameIdx = -1;
                }
            }
